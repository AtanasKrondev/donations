...
export class CausesService {
  ...
  load(id?: number) {
    return this.http.get<ICause[]>(`http://localhost:3000/causes${id ? `/${id}` : ''}`).pipe(
  ...
  donate(amount: number) {
    return this.http.put<ICause>(`http://localhost:3000/causes/${this.selectedCause.id}`, {
      cause: this.selectedCause.cause,
      description: this.selectedCause.description,
      neededAmount: this.selectedCause.neededAmount,
      imageUrl: this.selectedCause.imageUrl,
